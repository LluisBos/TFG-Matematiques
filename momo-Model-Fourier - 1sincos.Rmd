...
codi_lin_fourier <- nimbleCode({
    ...
    alphaF ~ dnorm(0, sd = 1000)
    betaF ~ dnorm(0, sd = 1000)
    ...
    for(t in 1:length(y)) {
        mu.y[t] <- beta0 + betaL*t + 
          alphaF * cos(2 * piVal *  yDay[t] / 365) + 
          betaF * sin(2 * piVal *  yDay[t] / 365)
...
ggplot(data = dades_pstr, aes(x = fecha_defuncion, y = defunciones_observadas)) +
  ...
  labs(x = "Data de Registre", y = "Nombre de Morts") +
  ggtitle("Model Lineal Harmònic") + # Títol
  scale_x_date(date_breaks = "1 year", date_labels = "%Y") +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))
```

```{r}
ggsave("model-lineal-harmonic-1sincos.pdf")
```
