---
title: "MOMO - HMM - Predicció Online"
...
initials <- readRDS("means-hmm-inits.rds")
Z_mod_est <- readRDS("Z-mode-hmm-inits.rds")
initials <- initials[!(names(initials) %in% c("Z", "probs"))]

#FALATARIA POSAR LES Z DEL HMM, Q ESTAN LES DE MIXTURES
...
if (ini > 1) {
  file_path <- paste0(dir_path, "/mcmc_pstr_hmm_temps_EQMP_", ini-1, ".rds")
...
for (i in ini:iter) {
  ...
  mcmc_output <- nimbleMCMC(codi_hmm, data = list(y = y, yDay = yD, DL = df$DL, DM = df$DM, DX = df$DX, DJ = df$DJ, DV = df$DV, DS = df$DS, alphas = c(1/2, 1/2, 1/2)),
                           constants = list(piVal = pi),
                           inits = initials,
                           monitors = c("beta0", "betaL", "alphaF", "betaF", "betaS", "betaM",
                                        "Gamma0", "Gamma", "Z", "y_pstr"), 
                           niter = 16000,
                           nburnin = 7000,
                           thin = 4,
                           nchains = 2)
  ...
  resum_dades <- MCMCsummary(mcmc_output, params = c("beta0", "betaL", "alphaF", "betaF", "betaS", "betaM", "Gamma0", "Gamma"))
  
  
  # Generació del nom del fitxer dinàmicament
  file_name1 <- paste0(dir_path, "/mcmc_pstr_hmm_summ_", i, ".rds")
  ...
  # Guardem en arxiu
  file_name2 <- paste0(dir_path, "/mcmc_pstr_hmm_Z_", i, ".rds")
  ...
  if (i > 1)
  {
    ...
    filename_y <- paste0(dir_path, "/mcmc_pstr_hmm_y_", i-1, ".rds")
  ...
  filename_y_s <- paste0(dir_path, "/mcmc_pstr_hmm_y_", i, ".rds")
  ...
  EQMP_temps_llista <- list(temps_comp = temps_comp, EQMP = EQMP)
   ...
   # Generació del nom del fitxer dinàmicament
  file_name3 <- paste0(dir_path, "/mcmc_pstr_hmm_temps_EQMP_", i, ".rds")
