...
mcmc_output <- nimbleMCMC(codi_hmm, data = list(y = y, yDay = yD, alphas = c(1/2, 1/2, 1/2), DL = dades_momo$DL, DM = dades_momo$DM, DX = dades_momo$DX, DJ = dades_momo$DJ, DV = dades_momo$DV, DS = dades_momo$DS),
                           ...
                           inits = initials,
                           monitors = c("beta0", "alphaF", "betaF", "betaL", 
                                        "betaM", "betaS", "Gamma", "Z", "y_pstr"), 
                           niter = 16000,
                           nburnin = 7500,
                           thin = 4,
                           nchains = 2)
...
```{r}
ggsave("grafica-hmm-nopred-Z.pdf", width = 10, height = 7)
```
...
```{r}
saveRDS(mcmc_output, "OUTPUTS/model-HMM-Setmana-NoPred-MCMC_output.RDS")
```
